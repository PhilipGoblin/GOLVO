...
#define LEFT 117    //servo setting (120 was previous setting, but the wheels collide with the wheel house if stötdämpare are loaded)
#define RIGHT 63    //servo setting (60 was previous setting)
#define SERVO_PIN 5
...
//#define ERROR_DISTANCE 666    NOT USED
...
/*
  if (distance_right < SAFETY_DISTANCE) {
    distance_right = SAFETY_DISTANCE;
  }

  if (distance_left < SAFETY_DISTANCE) {
    distance_left = SAFETY_DISTANCE;
  }
*/
   servo_position = ((distance_left - distance_right) + STRAIGHT);   //EX: 50 - 0 + 90 = 140   =
...
void setup() {
  ...
  /*  - SKRIV OM SERVO.WRITE OM DET FUNGERADE (rakställer hjulen innan programmet körs)                          */
  /************************************************************/
  Serial.begin(9600);
...
/*
  if (distance_front <= MIN_DISTANCE) {    //temporary if case, to easily stop the vehicle while testing on track. To be replaced with if case below when it works.
    brake();
  }
*/
  if (distance_front <= MIN_DISTANCE) {   //MAKE FUNCTION OF THIS
